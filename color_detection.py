...
def detect_color (frame, frame_HSV, name, lower, upper, colorBGR, cm_px_width=None, cm_px_height=None):
    ...
    if len(mask_contours) != 0:
        for mask_contour in mask_contours:
            if cv2.contourArea(mask_contour) > MIN_PIECE_AREA: # El contorno tiene que tener un tamaño mínimo
                ...
                if (cm_px_width and cm_px_height) is not None:
                    x_cm = cx * cm_px_width
                ...
                pieces.append({
                    "name": name,
                    "center_px": (cx, cy),
                    "center_cm": (cx, cy) if (cm_px_width and cm_px_height) is not None else None,
                    "angle": angle,
                    "colorBGR": colorBGR
                })
